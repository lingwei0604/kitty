	...
	}{
		{
			...
			appService{
				...
				ur: (func() UserRepository {
					...
					ur.On("DeleteDevices", mock.Anything, mock.Anything).Return(func(ctx context.Context, uid uint) []entity.Device {
				...
				preAllocator: func() PreAllocator {
					p := &mocks.PreAllocator{}
					p.On("ClearPreallocate", mock.Anything, mock.Anything, mock.Anything).Return(nil).Once()
					p.On("DeleteCacheByDevice", mock.Anything, mock.Anything, mock.Anything).Run(func(args mock.Arguments) {
						if id := args[2].(*entity.Device).ID; id != 1 {
...
func TestLogin(t *testing.T) {
		...
		},
		{
			...
			appService{
				...
				ur: (func() UserRepository {
					...
					ur.On("GetRecentUserByDevice", mock.Anything, mock.Anything, mock.Anything).Return(func(ctx context.Context, packageName string, device *entity.Device) *entity.User {
						return nil
					}, repository.ErrRecordNotFound).Once()
					return ur
				})(),
				cr: (func() CodeRepository {
					cr := &mocks.CodeRepository{}
	...
	userCache.On("CacheID", mock.Anything, mock.Anything).Return(nil)
	userCache.On("GetBindAd", mock.Anything, mock.Anything).Return(nil, nil)
	for _, c := range cases {
		cc := c
		cc.service.userCache = userCache
		t.Run(cc.name, func(t *testing.T) {
...
func TestUnbind(t *testing.T) {
	...
	app := appService{
		...
		ur: (func() UserRepository {
			...
			ur.On("DeleteDevices", mock.Anything, mock.Anything).Return([]entity.Device{
				{Model: gorm.Model{ID: 1}},
			}, nil)
			return ur
		})(),
		...
		preAllocator: func() PreAllocator {
			p := &mocks.PreAllocator{}
			p.On("ClearPreallocate", mock.Anything, mock.Anything, mock.Anything).Return(nil)
			p.On("DeleteCacheByDevice", mock.Anything, mock.Anything, mock.Anything).Run(func(args mock.Arguments) {
				if id := args[2].(*entity.Device).ID; id != 1 {
					t.Fatalf("get id %d", id)
				}
	...
	for _, c := range cases {
		...
		t.Run(cc.name, func(t *testing.T) {
			...
			ctx := context.WithValue(context.Background(), kitjwt.JWTClaimsContextKey, &claim)
			out, err := cc.app.Unbind(ctx, &cc.in)
			if err != nil {
				t.Fatal(err)
			}
			if out.Code != cc.out.Code {
...
func TestPreRegister(t *testing.T) {
	ur := &mocks.UserRepository{}
	ur.On("Get", mock.Anything, mock.Anything).Return(&entity.User{Model: gorm.Model{ID: 1}}, nil)
	ur.On("GetIDAndCreatedAtBySUUID", mock.Anything, mock.Anything, mock.Anything).Return(func(ctx context.Context, packageName string, suuid string) uint {
		if suuid == "1" {
			return 2
		}
		return 0
	}, time.Time{}, nil)
	ur.On("GetRecentUserByDevice", mock.Anything, mock.Anything, mock.Anything).Return(
		func(ctx context.Context, packageName string, device *entity.Device) *entity.User {
			user := &entity.User{}
			if !device.OaidEmpty() {
				user.ID = 3
				return user
	...
	pre := &mocks.PreAllocator{}
	pre.On("Preallocate", mock.Anything, mock.Anything, mock.Anything).Return(uint(5), nil)
	pre.On("SetIDByDevice", mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(nil)
	pre.On("GetIDByDevice", mock.Anything, mock.Anything, mock.Anything).Return(uint(0), nil)
	pre.On("SetRegisterTimeById", mock.Anything, mock.Anything, mock.Anything).Return(nil)
	pre.On("GetRegisterTimeById", mock.Anything, mock.Anything).Return(uint(0), nil)
	...
	cases := []struct {
		req    *pb.PreRegisterRequest
		wantId uint64
	}{
		{req: &pb.PreRegisterRequest{UserId: 1}, wantId: 1},
		{req: &pb.PreRegisterRequest{Suuid: "1"}, wantId: 2},
		{req: &pb.PreRegisterRequest{Oaid: "1"}, wantId: 3},
		{req: &pb.PreRegisterRequest{Idfa: "1"}, wantId: 4},
		{req: &pb.PreRegisterRequest{Suuid: "2"}, wantId: 5},
	}
	for _, c := range cases {
		r, _ := srv.PreRegister(context.Background(), c.req)
		assert.Equal(t, c.wantId, r.UserId)
	}
