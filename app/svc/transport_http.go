...
func MakeHTTPHandler(endpoints Endpoints, options ...httptransport.ServerOption) http.Handler {
	...
	m.Methods("POST").Path("/pre-register").Handler(httptransport.NewServer(
		endpoints.PreRegisterEndpoint,
		DecodeHTTPPreRegisterZeroRequest,
	...
	m.Methods("POST").Path("/login/wechat").Handler(httptransport.NewServer(
		endpoints.BindWechatEndpoint,
		DecodeHTTPBindWechatZeroRequest,
		EncodeHTTPGenericResponse,
		serverOptions...,
	))
	...
	m.Methods("GET").Path("/device").Handler(httptransport.NewServer(
		endpoints.DeviceLookupEndpoint,
		DecodeHTTPDeviceLookupZeroRequest,
		EncodeHTTPGenericResponse,
		serverOptions...,
	))
	return m
...
// DecodeHTTPPreRegisterZeroRequest is a transport/http.DecodeRequestFunc that
// decodes a JSON-encoded preregister request from the HTTP request
// body. Primarily useful in a server.
func DecodeHTTPPreRegisterZeroRequest(_ context.Context, r *http.Request) (interface{}, error) {
	defer r.Body.Close()
	var req pb.PreRegisterRequest
...
// DecodeHTTPBindWechatZeroRequest is a transport/http.DecodeRequestFunc that
// decodes a JSON-encoded bindwechat request from the HTTP request
// body. Primarily useful in a server.
func DecodeHTTPBindWechatZeroRequest(_ context.Context, r *http.Request) (interface{}, error) {
	defer r.Body.Close()
	var req pb.BindWechatRequest
	buf, err := ioutil.ReadAll(r.Body)
	if err != nil {
		return nil, errors.Wrapf(err, "cannot read body of http request")
	}
...
// DecodeHTTPDeviceLookupZeroRequest is a transport/http.DecodeRequestFunc that
// decodes a JSON-encoded devicelookup request from the HTTP request
// body. Primarily useful in a server.
func DecodeHTTPDeviceLookupZeroRequest(_ context.Context, r *http.Request) (interface{}, error) {
	defer r.Body.Close()
	var req pb.DeviceLookupRequest
	buf, err := ioutil.ReadAll(r.Body)
	if err != nil {
		return nil, errors.Wrapf(err, "cannot read body of http request")
	}
	...
	if OaidDeviceLookupStrArr, ok := queryParams["oaid"]; ok {
		OaidDeviceLookupStr := OaidDeviceLookupStrArr[0]
		OaidDeviceLookup := OaidDeviceLookupStr
		req.Oaid = OaidDeviceLookup
	}

	if ImeiDeviceLookupStrArr, ok := queryParams["imei"]; ok {
		ImeiDeviceLookupStr := ImeiDeviceLookupStrArr[0]
		ImeiDeviceLookup := ImeiDeviceLookupStr
		req.Imei = ImeiDeviceLookup
	}

	if PackageNameDeviceLookupStrArr, ok := queryParams["package_name"]; ok {
		PackageNameDeviceLookupStr := PackageNameDeviceLookupStrArr[0]
		PackageNameDeviceLookup := PackageNameDeviceLookupStr
		req.PackageName = PackageNameDeviceLookup
	}
