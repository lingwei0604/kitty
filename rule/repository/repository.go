...
func (r *repository) WatchConfigUpdate(ctx context.Context) error {
	...
	rch := r.client.Watch(ctx, OtherConfigPathPrefix, clientv3.WithPrefix(), clientv3.WithRev(r.rev))
...
func (r *repository) resetActiveContainers(ctx context.Context) error {
	...
	// Load central config
	{
		resp, err := r.client.Get(ctx, CentralConfigPath)
		if err != nil {
			return errors.Wrapf(err, "central config not found %s", OtherConfigPathPrefix)
		}
		for _, ev := range resp.Kvs {
			dbKey := CentralConfigPath
			name := "central-config"
			rule := entity.NewRules(bytes.NewReader(ev.Value), r.logger)
			newContainers[name] = Container{DbKey: dbKey, Name: name, RuleSet: rule}
		}
